...
use ratatui::{
    ...
    text::{Text, Line},
    symbols::border,
    widgets::{Block, Padding, Paragraph, Wrap, Widget},
    DefaultTerminal,
};
...
const TILDES: border::Set = border::Set {
    ...
    bottom_left: "~",
    vertical_left: "~",
    top_right: " ", bottom_right: " ", vertical_right: " ",
...
impl State {
    ...
    fn handlers(&mut self) -> io::Result<()> {
        if let Event::Key(key)=event::read()? {
            if key.kind==KeyEventKind::Press { self.exit=true; }
        }
        Ok(())
    }
}
impl Widget for &State {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let block = Block::bordered()
            ...
            .border_set(TILDES).border_style(Style::new().blue());

        text.block(block).render(layout[0],buf);
        Paragraph::new("0,0-1     All").alignment(Alignment::Right)
